...
指定されたタスクIDのスクリプトをkeruta APIから取得し、サブプロセスとして実行します。WebSocket通信により、タスク状態や標準入力、ログなどをリアルタイムで連携します。入力待ち状態を自動検出します。
...
2. WebSocketでkeruta APIに接続
3. タスク状態を`PROCESSING`に更新（WebSocketで即時反映）
4. 取得したスクリプトをサブプロセスとして実行
5. サブプロセスの標準出力・標準エラー出力をリアルタイムでキャプチャし、WebSocketで送信
6. サブプロセスが入力待ち状態になった場合、タスク状態を`WAITING_FOR_INPUT`に更新（WebSocketで即時反映）
7. 管理パネルからの標準入力送信をWebSocketで受信し、サブプロセスに反映
8. サブプロセスが正常終了した場合、タスク状態を`COMPLETED`に更新
9. サブプロセスが異常終了した場合、タスク状態を`FAILED`に更新
